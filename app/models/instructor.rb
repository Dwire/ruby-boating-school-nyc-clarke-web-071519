class Instructor
  ...
  def passed_student(student, name)
    student_tests = BoatingTest.all.select {|b_test| b_test.student == student}
    find_test = student_tests.find {|b_test| b_test.name == name}

    find_test.status = 'passed'
  end

  def failed_student(student, name)
    student_tests = BoatingTest.all.select {|b_test| b_test.student == student}
    find_test = student_tests.find {|b_test| b_test.name == name}
